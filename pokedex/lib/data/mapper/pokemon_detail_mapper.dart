...
import '../../application/constants/color_constants.dart';

extension PokemonDetailResponseMapper on PokemonDetailResponse? {
  PokemonDetail toDomain() {

    final types = this?.types?.map((type) => type.typeInfo?.name ?? StringConstants.empty).toList() ?? List.empty();
    ...
    return PokemonDetail(
      ...
      types: types,
      height: (this?.height ?? IntConstants.i0) / DoubleConstants.d10, // Convert decimetres to meters
      weight: (this?.weight ?? IntConstants.i0) / DoubleConstants.d10, // Convert hectograms to kilograms
